...
class VllmGeneratorModel(GeneratorModelBase):
    ...
    def generate_completions(
        ...
        import re
        if self.model_input_template_path_or_name in ["llama3_cot", "llama3_cot_defender"]:
            # outputs = [o.outputs[0].text for o in requests]
            violation_rate = 0
            end_early_rate = 0
            # index_end_early = []
            parsed = []
            ...
            for i, req in enumerate(requests):
                ...
                if req.outputs[0].finish_reason != "stop":
                    end_early_rate += 1
                    # index_end_early.append(i)
                if "<answer>" in o and not o.endswith("</answer>"):
                    o += " </answer>"
                match = ANSWER_PATTERN.search(o)
                if match:
                    parsed.append(match.group(1).strip())
                else:
                    violation_rate += 1
                    parsed.append(o) # keep the original output if no think tags foun
            print(f"\n{'='*50}")
            print(f"Violation rate: {violation_rate} / {len(requests)}")
            ...
            # since the truncate length is defined in number of tokens
            # we need to retokenize the outputs, truncate them, and then decode them
            outputs_tokens = [tokenizer.encode(o, add_special_tokens=False)[:truncate_length] for o in parsed] # remove BOS
