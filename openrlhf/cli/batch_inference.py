...
from openrlhf.datasets.prompts_dataset import postfillCOTPromptDataset
from openrlhf.models import Actor, get_llm_for_sequence_regression
from openrlhf.utils import blending_datasets, get_processor, get_strategy, get_tokenizer
from red_team.prompts import SELF_DISTILLATION_INSTRUCTION_PROMPT
...
def batch_generate_vllm(args):
    ...
    if args.apply_chat_template:
        raise NotImplementedError("apply_chat_template is not implemented for vllm")
    else:
        # Keep original prompts for output
        original_prompts = []
        for prompt, _ in prompts_dataset:
            original_prompts.append(prompt)

        messages = [[{"role": "user", "content": prompt}] for prompt in original_prompts]
    ...
    raw_outputs = llm.generate(prompts=templated_prompts * N, sampling_params=sampling_params)
    
    # Create a mapping from filtered outputs back to original prompts
    output_to_prompt_map = {}
    for idx, output in enumerate(raw_outputs):
        original_prompt_idx = idx // N  # Map back to original prompt index
        if output.outputs[0].finish_reason == "stop":
            if original_prompt_idx not in output_to_prompt_map:
                output_to_prompt_map[original_prompt_idx] = []
            output_to_prompt_map[original_prompt_idx].append(output)
    
    # Count filtered responses
    filtered_count = len(raw_outputs) - sum(len(outputs) for outputs in output_to_prompt_map.values())
    print(f"Filtered {filtered_count} unfinished responses")
    
    # Process outputs while maintaining correct alignment with original prompts
    for original_idx, outputs in output_to_prompt_map.items():
        for output in outputs:
            ...
            output_dataset.append({
                "vanilla": original_prompts[original_idx],  # Use original prompt without template
...
def batch_generate_vllm_postfill_cot(args):
    from vllm import LLM, SamplingParams
    from red_team.prompts import ASSISTANT_THINKING_PREFIX
    assistant_template = ASSISTANT_THINKING_PREFIX + "{thinking} </think>\n<answer> {sftchat_completion} </answer>"
    ...
    if args.apply_chat_template:
        ...
        # Instruction for COT
        postfill_instruction = SELF_DISTILLATION_INSTRUCTION_PROMPT

        messages = [[{"role": "user", "content": postfill_instruction.format(prompt=prompt, response=completion)}] for prompt,completion in original_prompts]
    ...
    for original_idx, outputs in output_to_prompt_map.items():
        for output in outputs:
            ...
            if output_text.endswith(tokenizer.eos_token):
                output_text = output_text[:-len(tokenizer.eos_token)]
            output_dataset.append({
                "vanilla": original_prompts[original_idx][0],  # Use original prompt without template
                "adversarial": "",
                "completion": assistant_template.format(thinking=output_text, sftchat_completion=original_prompts[original_idx][1]),
                "data_type":"vanilla_benign"
            })
...
if __name__ == "__main__":
    ...
    if args.eval_task and args.eval_task == "generate_vllm_postfill_cot":
        batch_generate_vllm_postfill_cot(args)
