...
class BasePPOTrainer(ABC):
    ...
    def __init__(
        ...
        **generate_kwargs,
        ...
        self.sftchat_loss_fn = GPTLMLoss()
        ...
        # Add validation for custom_configs
        if custom_configs is not None:
            required_keys = ["max_turns"]
            missing_keys = [key for key in required_keys if key not in custom_configs]
            if missing_keys:
                raise ValueError(f"custom_configs missing required keys: {missing_keys}")
            
            # Store the config
            self.custom_configs = custom_configs
        else:
            self.custom_configs = None
        ...
        self.replay_buffer = NaiveReplayBuffer(
            micro_train_batch_size, buffer_limit, buffer_cpu_offload, getattr(self.args, "packing_samples", False), self.custom_configs
        )
