...
END_OF_TURN_TOKEN = "<|start_header_id|>assistant<|end_header_id|>\n\n"

def preprocess_data(
    data, input_template=None, input_key="input", output_key=None, apply_chat_template=None, multiturn=False, prompt_input_template=None
):
    if apply_chat_template:
        if output_key:
            prompt_message = data[input_key]
            if prompt_input_template:
                prompt_message = prompt_input_template.format(user_query=prompt_message)
            ...
            # special case: abliterated model has a weird chat template
...
class SFTDataset(Dataset):
    ...
    def __init__(
        ...
        self.prompt_input_template = prompt_input_template
        # Get input/output keys from strategy args with fallbacks
        # First check for SFT-specific keys, then fall back to general keys
        self.input_key = getattr(self.strategy.args, "sft_input_key", 
                        getattr(self.strategy.args, "input_key", None))
        self.output_key = getattr(self.strategy.args, "sft_output_key", 
                         getattr(self.strategy.args, "output_key", None))
        
        if self.strategy.is_rank_0() and (self.input_key is None or (not self.pretrain_mode and self.output_key is None)):
            self.strategy.print(f"Warning: Using input_key={self.input_key}, output_key={self.output_key}")
